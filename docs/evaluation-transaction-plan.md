...
隔离 MySQL 8.4 验证并发发送一次、目标预算上限、整轮 70 次上限、跳号/重复序号拒绝、相同 execution ID 换 invocation 拒绝及事务中途异常全部回滚（6 项测试）。整轮上限测试以合成历史流水隔离预算条件，不代表完整 Run 调度验收。所有流水都计入预算，没有按失败/未知状态返还的接口。尚未验证实际模型调用次数；调用器只能在外层事务成功提交后使用发送许可，确认不明不能直接重放。
