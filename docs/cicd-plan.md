...
状态：已采纳。下文保留 2026-09-11 实施前的分析快照；当前实现以 [部署说明](../deploy/serverA/README.md) 为准，验收结果见 [部署验证](deployment-verification.md)。
...
本节所述现状对应方案形成时；后续实施与生产结果单独记录，不将规划当作运行证据。
...
2026-09-13 的 [checks 基线](https://github.com/FangcunMount/qs-ai/actions/runs/34738116139) 总历时约 18 分钟。MySQL 8.4 作业中 pytest 步骤 16 分 35 秒（1,349 passed，pytest 自报 992.37 秒），uv sync 2 秒、桥接构建/测试库初始化 4 秒；镜像作业总计约 53 秒。主要等待来自完整测试串行执行，不能据此判断线上模型执行慢。

CI 将每种 MySQL 的完整集合划分为 4 批独立 runner，每个 runner 拥有独立的 AI/QS 临时数据库；单批继续串行执行，避免全局资产、发布指针和用例清理并发冲突。全部原测试仍在两种 MySQL 上执行；这会增加初始化等 runner 总用量，以缩短墙钟等待时间。测试身份经稳定摘要排序后轮流分配，新增测试自动纳入。

每批输出完整收集清单、选择清单、实际通过/跳过记录、JUnit 和慢测试时间。`Complete test coverage` 汇总门禁要求同一提交、两种 MySQL、每种四批收集一致，所有用例恰好通过一次；缺批、漏测、重复、跳过或失败均不能通过工作流。部署仍只接受 main 对应提交的完整 checks 成功，不跳过合并后的精确版本验证。

同一个 PR 新提交到达时取消旧提交的 checks；main 检查和生产部署不会被此设置取消。预计等待可降至约 5–8 分钟，但须用实际新 CI 结果复核，不能把并行配置当成性能验收。初次本地分批/汇总测试 19 项通过，包括实际 pytest 子进程执行及缺失运行环境导致跳过时拒绝通过。

首轮实测：PR #74 源提交 `71b898cc8b89881ada176dce6114f542bc9729c4` 的 [checks 34739241483](https://github.com/FangcunMount/qs-ai/actions/runs/34739241483) 全部成功，05:00:51–05:07:39 UTC，共 6 分 48 秒；较上述 17 分 59 秒基线减少约 62%。汇总门禁及下载工件复核确认每种 MySQL 完整通过 1,370 项，包含新增跨语言和分批验证测试，无遗漏或跳过。工件绑定 GitHub PR 合并检验版本 `71bb98b7c09b617c0ca2d5bbcb694f5a29e0d517`，不能用 PR 分支 head 替换该 checkout 身份。

测试分批耗时为 141–304 秒；最慢条目主要是构造完整评测/发布证据的 fixture setup，单项约 7–9 秒。首轮证明并行缩短等待，尚未按耗时重新均衡批次；CI 总耗时不含合并后 main 的再次检查及实际部署。随后修正调度组：PR 按编号共享，main 按 run_id 独立，避免 main 作业或待运行检查互相排队/替换；该最终提交仍须单独通过检查。


2026-09-13 最终提速验证：PR #74 的最终提交 `84b654e5362787d3e5c91cf6a1a485dcf1a2d83b` 在 checks `34739626009` 全部通过，耗时 6 分 27 秒；已合并为 `b8ffb8ef50a2487c8792a8e924c44c852062e28c`。主干 checks `34739974760` 全部通过，耗时 6 分 12 秒。相对原 17 分 59 秒基线分别缩短约 64%/66%；保留每种 MySQL 的 1,370 项完整测试。以上是单次 CI 墙钟时间，实际等待会随 runner 负载变化，不包含 PR→主干→部署完整链路。纯 CI/测试/文档变更的发布运行 `34740229467` 仅 gate 成功，构建和部署跳过。
