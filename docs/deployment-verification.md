...
自动部署只在 main 合并提交相对第一父提交改变运行代码、配置、迁移、集成资产、镜像/依赖或部署脚本时继续构建和部署；纯文档、测试或 CI 检查配置变更保留完整 CI，但不触发生产重启。使用整个合并差异，不能因为 PR 最后一笔只是文档而漏掉之前的业务代码。删除/移动出运行目录也算运行变更。

手动 deploy/rollback 仍明确执行，并沿用 main 祖先与精确版本成功 CI 检查。范围判断不能证明旧失败部署已经补发；仍应根据实际 deployed SHA 对账并按需要手动部署已验证版本。
