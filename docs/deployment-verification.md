...
| `QS_AI_EXECUTION_ENABLED` | 启用单进程内部的用户解读循环；结果投递始终运行 |
| `QS_AI_EVALUATION_ENABLED` | 启用单进程内部的评测循环；只推进管理员已启动的评测，不自动启动待确认运行 |

`QS_AI_QS_ADDRESS` 始终必需，生产使用内网 `qs-apiserver:9090`，用于授权回查与可靠结果投递。生成或评测启用时要求 HTTPS `QS_AI_MODEL_ENDPOINT` 和 Secret `QS_AI_MODEL_API_KEY`。一个非特权、只读文件系统的 `qs-ai` 容器运行所有职责，使用统一 `/readyz` 检查；证书只挂载 CA 证书链、qs-ai 证书链与私钥。评测排空190秒，容器停止宽限210秒。完整生命周期、停止顺序和兼容回滚见[单进程说明](single-process.md)。

以下 M3/M4 初始化流程及分阶段证据是历史记录，旧容器名和命令不再用于当前部署。

集中发布先保持三个执行/管理开关关闭，完成兼容迁移并保存可回退基线；随后载入当前 v6 及管理所需引用，核对 QS 转发权限和评测套件。开启管理/评测，完成草稿、评测、审核和发布操作；确认当前发布配置后启用生成，完成授权案例和成果展示，再验证配置回退。QS 旧入口关闭前重新检查在途任务和当天预算。实际管理操作、真实生成、客户端与调试窗口验证仍分别验收（固定 24 小时等待已取消），本节是操作配置，不是完成证据。
...
## 2026-09-13 QS 生命周期代理发布

QS 主分支 CI 34727835812 与 [部署 34728242088](https://github.com/FangcunMount/qs-server/actions/runs/34728242088) 成功。独立 SSH 确认 serverA apiserver 与两份 collection 镜像均为 `63083526f6f6d64c4c2a2f2407f3d6c780a65e3c` 且 healthy。工作流报告另一主机 worker 发布和治理检查成功，本轮未独立 SSH 核验 worker。
