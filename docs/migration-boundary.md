...
新实现保留这些选择语义，并将不可变配置、发布记录、当前指针及每次变更审计分开。发布/替换必须明确确认当前指针版本和原 publication_id；回退引用历史 publication，不重新创建或改写旧配置。回退产生新的指针版本及审计，不能减小版本号。新请求解析生效指针，在接受任务的事务中冻结 publication_id、manifest、原始资产版本与摘要；在途任务和恢复执行按已冻结版本读取，不查询 latest。M5 分支已将发布绑定设为唯一接单/执行路径；旧固定配置执行失败。正式生成与评测开关继续独立控制，生产切换需单独验收。
...
新 Run 的 generation manifest 已在创建时冻结。带清单的执行现通过同一 MySQL 事务快照解析资产，核对完整清单及包字节摘要后才预留模型调用；生成结果接受时重新读取并验证，内容变化时保留待核对的 dispatch 状态。语义评测输入复用已核对的准备结果，模型路线也按 Run 中的确切引用读取。此改动移除执行环节对文件基线的依赖，但不扩展套件注册或省略质量门槛；新 Profile/套件管理及实际新版本评测仍待完成。无清单的旧 Run 保留原基线继续执行能力，发布仍受冻结清单门槛限制。

### Profile 配置写入权威

新增 Profile 版本注册入口及 MySQL 原命令回执事务，沿用既有策略定义、安全边界和 Schema，不引入另一份用户或权限体系。qs-ai 负责验证定义、确认资产、保存新版本和审计；QS 负责管理代理的业务授权，现已接入注册和原回执查询两项代理。此入口可以绑定已经冻结的新 Prompt，但只形成不可变配置，不能复用旧套件/旧 Run 批准来发布。未完成草稿编辑、套件管理和旧管理入口退役仍按 M3 清单推进；不会因新增此入口立刻删除 QS 旧路径。


### 原生套件注册与新版本评测

qs-ai 新增 `SuiteManagement` 和不可变套件登记，将保留案例及其全部质量义务绑定到新 Profile/Prompt/路线。登记、原命令回执、Run 创建、预检、生成/语义断言、质量门槛与发布执行均核对套件原文和清单。必须使用新的 Run 和批准；旧 Run、旧 suite 与旧审计不改写。当前覆盖已有案例合同的重新绑定，输入/维度筛选策略变化需要匹配的新案例合同，任意案例编辑仍待实现。

本批内部 RPC 尚未连接 QS 的套件管理授权代理和管理页面。全链路隔离验证使用合成事实、模型响应和审核，M1/M2 真实授权与生成验收仍未完成；不会切换生产写入权威或删除 QS 旧管理路径。接口、大小限制和历史兼容见 [原生评测套件](runtime.md#原生评测套件)。
