...
- 使用共享 selector 指针的唯一键和行锁，再锁定合法机构内的评测 Run/checkpoint；重新验证 approved 最终记录、全部历史轮次、G1–G5 和完整资产，不能只读取布尔 passed。
...
当前只迁入了发布选择、指针变更的纯规则和新 Run 清单冻结基础；发布表/变更事务、管理 RPC/REST、执行解析与在途冻结、草稿编辑/新套件以及旧写路径退役尚未完成。这些基础不构成发布权限或 M3 验收，实际生效仍受 M1/M2 先行验收和生产开关约束。
