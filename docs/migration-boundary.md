...
新增 Profile 版本注册入口及 MySQL 原命令回执事务，沿用既有策略定义、安全边界和 Schema，不引入另一份用户或权限体系。qs-ai 负责验证定义、确认资产、保存新版本和审计；QS 负责后续管理代理的业务授权。此入口可以绑定已经冻结的新 Prompt，但只形成不可变配置，不能复用旧套件/旧 Run 批准来发布。未完成草稿编辑、套件管理和旧管理入口退役仍按 M3 清单推进；不会因新增此入口立刻删除 QS 旧路径。
