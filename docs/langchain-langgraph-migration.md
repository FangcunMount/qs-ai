...
- M1：自定义 QSResponsesModel 通过 BaseChatModel.ainvoke 执行；原始字节严格解析、限制响应大小和单次发送保留，ambient LangSmith tracing 禁用。71 项模型相关检查通过。
- M2：发布配置通过工厂装配真实 prepare/generate/validate 图；应用层无框架导入。图取消、未知结果与并发隔离测试通过。
- M3：实际评测经过 prepare_dispatch/invoke_model/commit_receipt 图；数据库事务不跨模型请求，CAS 和既有检查点继续拥有恢复权。移除旧顺序编排及临时反向依赖，节点使用现有结构化日志。
- MySQL 8.4 专用临时库完成迁移：首轮生成/冻结配置/恢复 48 项通过；评测图及入口收敛后执行/恢复/worker/进程/未知结果 39 项通过。
- 完整非集成测试首跑 1227 通过、1 处旧私有字段测试失败；更新客户端关闭断言后对应测试及图/模型测试 16 项通过。全仓 mypy、ruff 与格式检查通过。
- 尚未完成：MySQL 8.0 和 Go 互操作 CI、最终镜像与生产部署、真实业务核证和回滚证据。以上不代表目标已完成。
