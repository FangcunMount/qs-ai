...
当前交付是只读观测入口和投递时间记录，尚未完成生产切流验收。它服务于 M4.4 的数值门槛、M4.5 的恢复检查及调试窗口观测，不替代真实管理、授权、模型输出与客户端验证。观察命令不修改发布指针、不创建测评、不调用模型。
...
固定 24 小时等待已取消；实际合法请求及正常/边界/缺失事实场景继续保留，调试与恢复验证通过后立即进入 M5。重大事实错误、越权、任务丢失、终态覆盖或未知调用被盲目重发时停止新准入。发布暂停后继续保留任务和资产，实际发布版本回退与业务配置回退分别记录。
