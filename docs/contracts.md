...
状态：HTTP 已落地创建、查询单会话、开始、回答、取消五个端点，以运行时 `/openapi.json` 为准。其余接口和新事实 RPC 仍为提案；已存在报告 RPC 仅生成传输探针。
...
当前五个端点中创建返回 201，开始/回答返回 202，取消返回 200。未实现列表、消息、重试、成果接口。默认身份和事实端口不可用，拒绝访问；测试替身不进入生产装配。
